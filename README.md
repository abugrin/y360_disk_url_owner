...
> Приложение предоставляется как есть и являются примером по работе с API для реализации собственных инструментов.
...
2. Введите OAuth токен для доступа к Яндекс 360 API (токен не будет отображаться при вводе)
3. Приложение проверит токен через API и покажет ваш логин и доступные организации
4. Приложение проверит наличие необходимых прав доступа (scope)
5. Введите ID организации (org_id) из списка доступных
...
4. Для выхода введите `q`, `quit` или `exit`
